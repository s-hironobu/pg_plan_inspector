 ...
 * pgqp_explain.c
 *
 * This is based on src/backend/commands/explain.c, and it is significantly
 * modified to use with pg_query_plan module.
...
extern bool pgqp_received_signal;
#ifdef __ADDITIONAL_OPTIONS__
extern bool pgqp_log_buffers;
extern bool pgqp_log_wal;
#endif
#endif							/* __PG_QUERY_PLAN__ */
...
void		pgqpExplainPrintQueryState(ExplainState *es, QueryDesc *queryDesc);
void		pgqpExplainBeginOutput(ExplainState *es);
void		pgqpExplainEndOutput(ExplainState *es);
#endif							/* __PG_QUERY_PLAN__ */
...
#ifdef __PG_QUERY_PLAN__
/*
 * Add current data, such as nloops and rows, to es->str
 */
static void
ExplainAddState(Instrumentation *instrument, ExplainState *es)
{
	...
	if (nloops == 0)
	{
		startup_ms = 1000.0 * instrument->firsttuple;
		total_ms = 1000.0 * INSTR_TIME_GET_DOUBLE(instrument->counter);
		rows = instrument->tuplecount;
	}
	else
	{
	...
	else
	{
		...
		if (pgqp_log_buffers)
		{
			ExplainOpenGroup("BufferUsage_Start", "BufferUsage_Start", true, es);

			ExplainPropertyFloat("shared_blks_hit",
								 NULL, instrument->bufusage_start.shared_blks_hit, 0, es);
			ExplainPropertyFloat("shared_blks_read",
								 NULL, instrument->bufusage_start.shared_blks_read, 0, es);
			ExplainPropertyFloat("shared_blks_dirtied",
								 NULL, instrument->bufusage_start.shared_blks_dirtied, 0, es);
			ExplainPropertyFloat("shared_blks_written",
								 NULL, instrument->bufusage_start.shared_blks_written, 0, es);
			ExplainPropertyFloat("local_blks_hit",
								 NULL, instrument->bufusage_start.local_blks_hit, 0, es);
			ExplainPropertyFloat("local_blks_read",
								 NULL, instrument->bufusage_start.local_blks_read, 0, es);
			ExplainPropertyFloat("local_blks_dirtied",
								 NULL, instrument->bufusage_start.local_blks_dirtied, 0, es);
			ExplainPropertyFloat("local_blks_written",
								 NULL, instrument->bufusage_start.local_blks_written, 0, es);
			ExplainPropertyFloat("temp_blks_read",
								 NULL, instrument->bufusage_start.temp_blks_read, 0, es);
			ExplainPropertyFloat("temp_blks_written",
								 NULL, instrument->bufusage_start.temp_blks_written, 0, es);
			ExplainPropertyFloat("blk_read_time", "s",
								 1000.0 * INSTR_TIME_GET_DOUBLE(instrument->bufusage_start.blk_read_time), 3, es);
			ExplainPropertyFloat("blk_write_time", "s",
								 1000.0 * INSTR_TIME_GET_DOUBLE(instrument->bufusage_start.blk_write_time), 3, es);

			ExplainCloseGroup("BufferUsage_Start", "BufferUsage_Start", true, es);
		}

		if (pgqp_log_wal)
		{
			ExplainOpenGroup("WalUsage_Start", "WalUsage_Start", true, es);

			ExplainPropertyFloat("wal_records",
								 NULL, instrument->walusage_start.wal_records, 0, es);
			ExplainPropertyFloat("wal_fpi",
								 NULL, instrument->walusage_start.wal_fpi, 0, es);
			ExplainPropertyInteger("wal_bytes", NULL, instrument->walusage_start.wal_bytes, es);

			ExplainCloseGroup("WalUsage_Start", "WalUsage_Start", true, es);
		}

		if (pgqp_log_buffers)
		{
			ExplainOpenGroup("BufferUsage", "BufferUsage", true, es);

			ExplainPropertyFloat("shared_blks_hit",
								 NULL, instrument->bufusage.shared_blks_hit, 0, es);
			ExplainPropertyFloat("shared_blks_read",
								 NULL, instrument->bufusage.shared_blks_read, 0, es);
			ExplainPropertyFloat("shared_blks_dirtied",
								 NULL, instrument->bufusage.shared_blks_dirtied, 0, es);
			ExplainPropertyFloat("shared_blks_written",
								 NULL, instrument->bufusage.shared_blks_written, 0, es);
			ExplainPropertyFloat("local_blks_hit",
								 NULL, instrument->bufusage.local_blks_hit, 0, es);
			ExplainPropertyFloat("local_blks_read",
								 NULL, instrument->bufusage.local_blks_read, 0, es);
			ExplainPropertyFloat("local_blks_dirtied",
								 NULL, instrument->bufusage.local_blks_dirtied, 0, es);
			ExplainPropertyFloat("local_blks_written",
								 NULL, instrument->bufusage.local_blks_written, 0, es);
			ExplainPropertyFloat("temp_blks_read",
								 NULL, instrument->bufusage.temp_blks_read, 0, es);
			ExplainPropertyFloat("temp_blks_written",
								 NULL, instrument->bufusage.temp_blks_written, 0, es);
			ExplainPropertyFloat("blk_read_time", "s",
								 1000.0 * INSTR_TIME_GET_DOUBLE(instrument->bufusage.blk_read_time), 3, es);
			ExplainPropertyFloat("blk_write_time", "s",
								 1000.0 * INSTR_TIME_GET_DOUBLE(instrument->bufusage.blk_write_time), 3, es);

			ExplainCloseGroup("BufferUsage", "BufferUsage", true, es);
		}

		if (pgqp_log_wal)
		{
			ExplainOpenGroup("WalUsage", "WalUsage", true, es);
			...
			ExplainCloseGroup("WalUsage", "WalUsage", true, es);
		}
#endif							/* __ADDITIONAL_OPTIONS__ */
	}
}
#endif							/* __PG_QUERY_PLAN__ */
...
#ifdef __PG_QUERY_PLAN__
/*
 * ExplainPrintPlanState
 *
 */
void
pgqpExplainPrintQueryState(ExplainState *es, QueryDesc *queryDesc)
#else
/*
 * ExplainPrintPlan -
			...
			ExplainState *es)
{
	...
	{
		/* Add query progress */
		ExplainAddState(planstate->instrument, es);
...
#ifndef __PG_QUERY_PLAN__		/* We currently omit the JIT info. */
...
#ifndef __PG_QUERY_PLAN__		/* We currently omit the buffer/WAL usage. */
...
#ifndef __PG_QUERY_PLAN__		/* We currently omit the per-worker's
								 * buffer/WAL usage. */
...
show_sort_info(SortState *sortstate, ExplainState *es)
{
	...
	/* We currently omit per-worker's sort usage. */
#ifndef __PG_QUERY_PLAN__
						   ...
						   ExplainState *es)
{
	...
	/* We currently omit per-worker's incremental sort usage. */
#ifndef __PG_QUERY_PLAN__
...
show_memoize_info(MemoizeState *mstate, List *ancestors, ExplainState *es)
{
	...
	/* We currently omit per-worker's cache usage. */
#ifndef __PG_QUERY_PLAN__
...
show_hashagg_info(AggState *aggstate, ExplainState *es)
{
	...
	/* We currently omit per-worker's hashAgg usage. */
#ifndef __PG_QUERY_PLAN__
...
#ifdef __PG_QUERY_PLAN__
void
pgqpExplainBeginOutput(ExplainState *es)
#else
void
...
#ifdef __PG_QUERY_PLAN__
void
pgqpExplainEndOutput(ExplainState *es)
#else
void
