...
void
#ifdef __PG_QUERY_PLAN__
pgqp_create_index_paths(PlannerInfo *root, RelOptInfo *rel)
#else
...
#ifdef __PG_QUERY_PLAN__
		bpath = pgqp_create_bitmap_heap_path(root, rel, bitmapqual,
...
#ifdef __PG_QUERY_PLAN__
			pgqp_create_partial_bitmap_paths(root, rel, bitmapqual);
#else
				  ...
				  bool *skip_lower_saop)
{
	...
	{
#ifdef __PG_QUERY_PLAN__
		ipath = pgqp_create_index_path(root, index,
									   index_clauses,
									   orderbyclauses,
									   orderbyclausecols,
									   useful_pathkeys,
						 ...
						 List *clauses, List *other_clauses)
{
	...
	foreach(lc, clauses)
	{
		...
		{
#ifdef __PG_QUERY_PLAN__
			bitmapqual = (Path *) pgqp_create_bitmap_or_path(root, rel, pathlist);
#else
...
#ifdef __PG_QUERY_PLAN__
			pgqp_cost_bitmap_tree_node(pathinfo->path, &ncost, &nselec);
			pgqp_cost_bitmap_tree_node(pathinfoarray[i]->path, &ocost, &oselec);
#else
...
#ifdef __PG_QUERY_PLAN__
	return (Path *) pgqp_create_bitmap_and_path(root, rel, bestpaths);
#else
...
#ifdef __PG_QUERY_PLAN__
	pgqp_cost_bitmap_tree_node(pa->path, &acost, &aselec);
	pgqp_cost_bitmap_tree_node(pb->path, &bcost, &bselec);
#else
...
#ifdef __PG_QUERY_PLAN__
	pgqp_cost_bitmap_heap_scan(&bpath.path, root, rel,
...
bitmap_and_cost_est(PlannerInfo *root, RelOptInfo *rel, List *paths)
{
	 ...
	 */
#ifdef __PG_QUERY_PLAN__
	apath = pgqp_create_bitmap_and_path(root, rel, paths);
#else
