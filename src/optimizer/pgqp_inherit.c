...
void
#ifdef __PG_QUERY_PLAN__
pgqp_expand_inherited_rtentry(PlannerInfo *root, RelOptInfo *rel,
							  RangeTblEntry *rte, Index rti)
#else
...
#ifdef __PG_QUERY_PLAN__
		childrc->markType = pgqp_select_rowmark_type(childrte,
						  ...
						  RangeTblEntry *rte, Index rti)
{
	...
	foreach(l, root->append_rel_list)
	{
		...
		if (childrte->inh)
#ifdef __PG_QUERY_PLAN__
			pgqp_expand_inherited_rtentry(root, childrel, childrte, childRTindex);
#else
