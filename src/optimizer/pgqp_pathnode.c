...
Path *
#ifdef __PG_QUERY_PLAN__
pgqp_create_seqscan_path(PlannerInfo *root, RelOptInfo *rel,
...
#ifdef __PG_QUERY_PLAN__
	pathnode->param_info = pgqp_get_baserel_parampathinfo(root, rel,
														  required_outer);
#else
...
Path *
#ifdef __PG_QUERY_PLAN__
pgqp_create_samplescan_path(PlannerInfo *root, RelOptInfo *rel, Relids required_outer)
#else
...
IndexPath *
#ifdef __PG_QUERY_PLAN__
pgqp_create_index_path(PlannerInfo *root,
					   IndexOptInfo *index,
					   List *indexclauses,
					   List *indexorderbys,
...
#ifdef __PG_QUERY_PLAN__
	pathnode->path.param_info = pgqp_get_baserel_parampathinfo(root, rel,
															   required_outer);
#else
...
#ifdef __PG_QUERY_PLAN__
	pgqp_cost_index(pathnode, root, loop_count, partial_path);
#else
...
BitmapHeapPath *
#ifdef __PG_QUERY_PLAN__
pgqp_create_bitmap_heap_path(PlannerInfo *root,
							 RelOptInfo *rel,
							 Path *bitmapqual,
							 Relids required_outer,
							 double loop_count,
...
#ifdef __PG_QUERY_PLAN__
	pgqp_cost_bitmap_heap_scan(&pathnode->path, root, rel,
...
BitmapAndPath *
#ifdef __PG_QUERY_PLAN__
pgqp_create_bitmap_and_path(PlannerInfo *root,
							RelOptInfo *rel,
							List *bitmapquals)
#else
...
#ifdef __PG_QUERY_PLAN__
	pgqp_cost_bitmap_and_node(pathnode, root);
#else
...
BitmapOrPath *
#ifdef __PG_QUERY_PLAN__
pgqp_create_bitmap_or_path(PlannerInfo *root,
						   RelOptInfo *rel,
						   List *bitmapquals)
#else
...
#ifdef __PG_QUERY_PLAN__
	pgqp_cost_tidrangescan(&pathnode->path, root, rel, tidrangequals,
						   pathnode->path.param_info);
#else
...
AppendPath *
#ifdef __PG_QUERY_PLAN__
pgqp_create_append_path(PlannerInfo *root,
						RelOptInfo *rel,
...
GatherMergePath *
#ifdef __PG_QUERY_PLAN__
pgqp_create_gather_merge_path(PlannerInfo *root, RelOptInfo *rel, Path *subpath,
...
GatherPath *
#ifdef __PG_QUERY_PLAN__
pgqp_create_gather_path(PlannerInfo *root, RelOptInfo *rel, Path *subpath,
...
Path *
#ifdef __PG_QUERY_PLAN__
pgqp_create_namedtuplestorescan_path(PlannerInfo *root, RelOptInfo *rel,
									 Relids required_outer)
#else
...
NestPath *
#ifdef __PG_QUERY_PLAN__
pgqp_create_nestloop_path(PlannerInfo *root,
						  RelOptInfo *joinrel,
						  JoinType jointype,
...
MergePath *
#ifdef __PG_QUERY_PLAN__
pgqp_create_mergejoin_path(PlannerInfo *root,
						   RelOptInfo *joinrel,
						   JoinType jointype,
						   JoinCostWorkspace *workspace,
						   JoinPathExtraData *extra,
...
#ifdef __PG_QUERY_PLAN__
	pgqp_final_cost_mergejoin(root, pathnode, workspace, extra);
#else
...
HashPath *
#ifdef __PG_QUERY_PLAN__
pgqp_create_hashjoin_path(PlannerInfo *root,
						  RelOptInfo *joinrel,
						  JoinType jointype,
						  JoinCostWorkspace *workspace,
						  JoinPathExtraData *extra,
...
#ifdef __PG_QUERY_PLAN__
	pgqp_final_cost_hashjoin(root, pathnode, workspace, extra);
#else
...
Path *
#ifdef __PG_QUERY_PLAN__
pgqp_reparameterize_path(PlannerInfo *root, Path *path,
						 Relids required_outer,
						 double loop_count)
#else
...
#endif
{
	...
	switch (path->pathtype)
	{
		case T_SeqScan:
#ifdef __PG_QUERY_PLAN__
			return pgqp_create_seqscan_path(root, rel, required_outer, 0);
#else
...
#ifdef __PG_QUERY_PLAN__
				newpath->path.param_info =
					pgqp_get_baserel_parampathinfo(root, rel, required_outer);
				pgqp_cost_index(newpath, root, loop_count, false);
#else
				newpath->path.param_info =
					get_baserel_parampathinfo(root, rel, required_outer);
...
#ifdef __PG_QUERY_PLAN__
				return (Path *) pgqp_create_bitmap_heap_path(root,
															 rel,
															 bpath->bitmapqual,
															 required_outer,
															 loop_count, 0);
