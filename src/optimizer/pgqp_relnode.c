...
RelOptInfo *
#ifdef  __PG_QUERY_PLAN__
pgqp_build_join_rel(PlannerInfo *root,
					Relids joinrelids,
					RelOptInfo *outer_rel,
					RelOptInfo *inner_rel,
					SpecialJoinInfo *sjinfo,
...
#endif
{
	 ...
	 */
#ifdef __PG_QUERY_PLAN__
	pgqp_set_joinrel_size_estimates(root, joinrel, outer_rel, inner_rel,
									sjinfo, restrictlist);
#else
...
#ifdef __DEBUG__
	elog_node_display(LOG, "TEST BEFORE  adjust_joinrel_rows in build_join_rel", joinrel, true);
#endif

	/*
	 * Adjusting rows should be done before setting consider_parallel flag.
	 */
	if (!adjust_joinrel_rows(joinrel, outer_rel, inner_rel, false))
	{
		if (!adjust_joinrel_rows(joinrel, inner_rel, outer_rel, true))
			elog(DEBUG3, "Not adjusted rows.");
	}
#endif							/* __PG_QUERY_PLAN__ */
...
#ifdef __DEBUG__
	elog_node_display(LOG, "TEST AFTER  add_join_rel  in build_join_rel", joinrel, true);
#endif
...
RelOptInfo *
#ifdef __PG_QUERY_PLAN__
pgqp_build_child_join_rel(PlannerInfo *root, RelOptInfo *outer_rel,
...
ParamPathInfo *
#ifdef __PG_QUERY_PLAN__
pgqp_get_baserel_parampathinfo(PlannerInfo *root, RelOptInfo *baserel,
...
#ifdef __PG_QUERY_PLAN__
	rows = pgqp_get_parameterized_baserel_size(root, baserel, pclauses);
#else
...
ParamPathInfo *
#ifdef __PG_QUERY_PLAN__
pgqp_get_joinrel_parampathinfo(PlannerInfo *root, RelOptInfo *joinrel,
							   Path *outer_path,
							   Path *inner_path,
							   SpecialJoinInfo *sjinfo,
							   Relids required_outer,
...
#ifdef __PG_QUERY_PLAN__
	rows = pgqp_get_parameterized_joinrel_size(root, joinrel,
											   outer_path,
											   inner_path,
											   sjinfo,
											   *restrict_clauses);
