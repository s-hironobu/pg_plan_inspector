...
RelOptInfo *
#ifdef __PG_QUERY_PLAN__
pgqp_plan_set_operations(PlannerInfo *root)
#else
					   ...
					   double *pNumGroups)
{
	...
	if (IsA(setOp, RangeTblRef))
	{
		...
		*pTargetList = tlist;
		 ...
		 */
#ifdef __PG_QUERY_PLAN__
		subpath = pgqp_get_cheapest_fractional_path(final_rel,
													root->tuple_fraction);
#else
					 ...
					 List **pTargetList)
{
	...
	*pTargetList = tlist;
	 ...
	 */
#ifdef __PG_QUERY_PLAN__
	path = (Path *) pgqp_create_append_path(root, result_rel, pathlist, NIL,
											NIL, NULL, 0, false, -1);
#else
	...
	if (partial_paths_valid)
	{
		...
		ppath = (Path *)
#ifdef __PG_QUERY_PLAN__
			pgqp_create_append_path(root, result_rel, NIL, partial_pathlist,
									NIL, NULL,
		...
		ppath = (Path *)
#ifdef __PG_QUERY_PLAN__
			pgqp_create_gather_path(root, result_rel, ppath,
