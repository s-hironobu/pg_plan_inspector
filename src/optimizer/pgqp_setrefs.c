...
Plan *
#ifdef __PG_QUERY_PLAN__
pgqp_set_plan_references(PlannerInfo *root, Plan *plan)
#else
...
convert_combining_aggrefs(Node *node, void *context)
{
	...
	if (IsA(node, Aggref))
	{
		 ...
		 */
#ifdef __PG_QUERY_PLAN__
		pgqp_mark_partial_aggref(child_agg, AGGSPLIT_INITIAL_SERIAL);
#else
		...
		parent_agg->args = list_make1(makeTargetEntry((Expr *) child_agg,
													  1, NULL, false));
#ifdef __PG_QUERY_PLAN__
		pgqp_mark_partial_aggref(parent_agg, AGGSPLIT_FINAL_DESERIAL);
#else
 ...
 *
 * This is used in three/four different scenarios:
 ...
 *	  relation.
 * 4) MERGE.(ver 15 or later)  In this case, references to the source relation are to be
