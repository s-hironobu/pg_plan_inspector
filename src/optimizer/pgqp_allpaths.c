...
#include "pgqp_indexpath.h"
#include "pgqp_joinrels.h"
...
#ifdef __PG_QUERY_PLAN__
RelOptInfo *
pgqp_make_one_rel(PlannerInfo *root, List *joinlist)
#else
RelOptInfo *
...
#ifdef __PG_QUERY_PLAN__
	add_path(rel, pgqp_create_seqscan_path(root, rel, required_outer, 0));
#else
...
#ifdef __PG_QUERY_PLAN__
	pgqp_create_index_paths(root, rel);
...
#ifdef __PG_QUERY_PLAN__
	add_partial_path(rel, pgqp_create_seqscan_path(root, rel, NULL, parallel_workers));
#else
...
#ifdef __PG_QUERY_PLAN__
	path = pgqp_create_samplescan_path(root, rel, required_outer);
#else
...
#ifdef __PG_QUERY_PLAN__
	pgqp_add_paths_to_append_rel(root, rel, live_childrels);
#else
...
void
#ifdef __PG_QUERY_PLAN__
pgqp_add_paths_to_append_rel(PlannerInfo *root, RelOptInfo *rel,
							 List *live_childrels)
#else
...
#endif
{
	...
	if (subpaths_valid)
#ifdef __PG_QUERY_PLAN__
		add_path(rel, (Path *) pgqp_create_append_path(root, rel, subpaths, NIL,
													   NIL, NULL, 0, false,
													   -1));
#else
...
#ifdef __PG_QUERY_PLAN__
		appendpath = pgqp_create_append_path(root, rel, NIL, partial_subpaths,
...
#ifdef __PG_QUERY_PLAN__
		appendpath = pgqp_create_append_path(root, rel, pa_nonpartial_subpaths,
											 pa_partial_subpaths,
...
#ifdef __PG_QUERY_PLAN__
			add_path(rel, (Path *)
					 pgqp_create_append_path(root, rel, subpaths, NIL,
...
#ifdef __PG_QUERY_PLAN__
			appendpath = pgqp_create_append_path(root, rel, NIL, list_make1(path),
												 NIL, NULL,
									  ...
									  Relids required_outer)
{
	...
	foreach(lc, rel->pathlist)
	{
		...
		{
#ifdef __PG_QUERY_PLAN__
			path = pgqp_reparameterize_path(root, path, required_outer, 1.0);
#else
...
set_dummy_rel_pathlist(RelOptInfo *rel)
{
	...
	/* Not use pgqp_create_append_path because of dummy */
#ifdef __PG_QUERY_PLAN__
	add_path(rel, (Path *) pgqp_create_append_path(NULL, rel, NIL, NIL,
					  ...
					  Index rti, RangeTblEntry *rte)
{
	...
	foreach(lc, sub_final_rel->pathlist)
	{
		...
		add_path(rel, (Path *)
#ifdef __PG_QUERY_PLAN__
				 pgqp_create_subqueryscan_path(root, rel, subpath,
#if PG_VERSION_NUM >= 160000
											   trivial_pathtarget,
#endif
							 ...
							 RangeTblEntry *rte)
{
	...
	/* Generate appropriate path */
#ifdef __PG_QUERY_PLAN__
	add_path(rel, pgqp_create_namedtuplestorescan_path(root, rel, required_outer));
#else
					...
					RangeTblEntry *rte)
{
	...
	/* Mark rel with estimated output rows, width, etc */
#ifdef __PG_QUERY_PLAN__
	pgqp_set_result_size_estimates(root, rel);
#else
...
void
#ifdef __PG_QUERY_PLAN__
pgqp_generate_gather_paths(PlannerInfo *root, RelOptInfo *rel, bool override_rows)
#else
...
#ifdef __PG_QUERY_PLAN__
		pgqp_create_gather_path(root, rel, cheapest_partial_path, rel->reltarget,
								NULL, rowsp);
#else
...
#ifdef __PG_QUERY_PLAN__
		path = pgqp_create_gather_merge_path(root, rel, subpath, rel->reltarget,
...
void
#ifdef __PG_QUERY_PLAN__
pgqp_generate_useful_gather_paths(PlannerInfo *root, RelOptInfo *rel, bool override_rows)
#else
...
#ifdef __PG_QUERY_PLAN__
	pgqp_generate_gather_paths(root, rel, override_rows);
#else
		...
		foreach(lc2, rel->partial_pathlist)
		{
			...
			if (presorted_keys == 0 || !enable_incremental_sort)
			{
				...
				subpath = (Path *) create_incremental_sort_path(root,
																...
																-1);
#ifdef __PG_QUERY_PLAN__
			path = pgqp_create_gather_merge_path(root, rel,
												 subpath,
												 rel->reltarget,
												 subpath->pathkeys,
												 NULL,
...
#ifdef __PG_QUERY_PLAN__
			return pgqp_standard_join_search(root, levels_needed, initial_rels);
#else
...
RelOptInfo *
#ifdef __PG_QUERY_PLAN__
pgqp_standard_join_search(PlannerInfo *root, int levels_needed, List *initial_rels)
#else
...
#endif
{
	...
	for (lev = 2; lev <= levels_needed; lev++)
	{
		 ...
		 */
#ifdef __PG_QUERY_PLAN__
		pgqp_join_search_one_level(root, lev);
#else
...
#ifdef __PG_QUERY_PLAN__
			pgqp_generate_partitionwise_join_paths(root, rel);
#else
...
void
#ifdef __PG_QUERY_PLAN__
pgqp_create_partial_bitmap_paths(PlannerInfo *root, RelOptInfo *rel,
								 Path *bitmapqual)
#else
...
#ifdef __PG_QUERY_PLAN__
	pages_fetched = pgqp_compute_bitmap_pages(root, rel, bitmapqual, 1.0,
											  NULL, NULL);
#else
...
#ifdef __PG_QUERY_PLAN__
	add_partial_path(rel, (Path *) pgqp_create_bitmap_heap_path(root, rel,
...
void
#ifdef __PG_QUERY_PLAN__
pgqp_generate_partitionwise_join_paths(PlannerInfo *root, RelOptInfo *rel)
#else
...
#ifdef __PG_QUERY_PLAN__
		pgqp_generate_partitionwise_join_paths(root, child_rel);
#else
...
#ifdef __PG_QUERY_PLAN__
	pgqp_add_paths_to_append_rel(root, rel, live_children);
#else
