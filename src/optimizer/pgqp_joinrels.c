...
#include "pgqp_relnode.h"
#include "pgqp_joinpath.h"
...
void
#ifdef __PG_QUERY_PLAN__
pgqp_join_search_one_level(PlannerInfo *root, int level)
#else
		...
		foreach(r, joinrels[k])
		{
			...
			for_each_cell(r2, other_rels_list, other_rels)
			{
				...
				if (!bms_overlap(old_rel->relids, new_rel->relids))
				{
					...
					{
#ifdef __PG_QUERY_PLAN__
						(void) pgqp_make_join_rel(root, old_rel, new_rel);
#else
						  ...
						  ListCell *other_rels)
{
	...
	for_each_cell(l, other_rels_list, other_rels)
	{
		...
		{
#ifdef __PG_QUERY_PLAN__
			(void) pgqp_make_join_rel(root, old_rel, other_rel);
#else
...
RelOptInfo *
#ifdef  __PG_QUERY_PLAN__
pgqp_make_join_rel(PlannerInfo *root, RelOptInfo *rel1, RelOptInfo *rel2)
#else
...
#endif
{
	...
	int			save_nestlevel;
#ifdef __DEBUG__
	char		_rel1_relids[32];
	char		_rel2_relids[32];

	show_relids(rel1->relids, _rel1_relids);
	show_relids(rel2->relids, _rel2_relids);
	elog(LOG, "TEST %s    rel1=(%s)   rel2=(%s)",
		 __func__, _rel1_relids, _rel2_relids);
#endif

	save_nestlevel = NewGUCNestLevel();

	/*
	 * First of all, we set all join-methods to disable. After that, in each
	 * join level, we set the specified join-method by the regression
	 * parameters.
	 */
					   ...
					   &sjinfo, &reversed))
	{
		/* invalid join path */
#ifdef __DEBUG__
		elog(LOG, "TEST %s  return NULL after join_is_legal()", __func__);
#endif
		bms_free(joinrelids);
		return NULL;
	}
							...
							SpecialJoinInfo *sjinfo, List *restrictlist)
{
	...
	switch (sjinfo->jointype)
	{
		case JOIN_INNER:
			...
			{
#ifdef __DEBUG__
				elog(LOG, "TEST %s   JOIN_INNER ==> mark_dummy_rel(joinrel)", __func__);
#endif
				mark_dummy_rel(joinrel);
				break;
			}
#ifdef __DEBUG__
			elog(LOG, "TEST %s   JOIN_INNER", __func__);
#endif

#ifdef __PG_QUERY_PLAN__
			if (pgqp_adjust_rows
				&& pgqp_enable_adjust_joinrel_rows
				&& check_join_param(&join_type, &mergeflag, rel1->relids, rel2->relids))
			{
				join_mask = get_join_mask(join_type);
				set_join_config_options(join_mask, false,
										current_state.context);
				pgqp_add_paths_to_joinrel(root, joinrel, rel1, rel2,
										  JOIN_INNER, sjinfo,
										  restrictlist);
			}
			else if (pgqp_adjust_rows
					 && pgqp_enable_adjust_joinrel_rows
					 && check_join_param(&join_type, &mergeflag, rel1->relids, rel2->relids))
			{
				join_mask = get_join_mask(join_type);
		...
		case JOIN_LEFT:
			...
			{
#ifdef __DEBUG__
				elog(LOG, "TEST %s   JOIN_LEFT  => mark_dummy_rel(joinrel)", __func__);
#endif
				mark_dummy_rel(joinrel);
				break;
			}
			...
			{
#ifdef __DEBUG__
				elog(LOG, "TEST %s   JOIN_LEFT  => mark_dummy_rel(rel2)", __func__);
#endif
				mark_dummy_rel(rel2);
			}
#ifdef __DEBUG__
			elog(LOG, "TEST %s   JOIN_LEFT", __func__);
#endif
				...
				&& check_join_param(&join_type, &mergeflag, rel1->relids, rel2->relids))
			{
				...
				pgqp_add_paths_to_joinrel(root, joinrel, rel1, rel2,
					 ...
					 && pgqp_enable_adjust_joinrel_rows
					 && check_join_param(&join_type, &mergeflag, rel2->relids, rel1->relids))
			{
				join_mask = get_join_mask(join_type);
				set_join_config_options(join_mask, false,
										current_state.context);
		...
		case JOIN_FULL:
			...
			{
#ifdef __DEBUG__
				elog(LOG, "TEST %s   JOIN_FULL  => mark_dummy_rel(joinrel)", __func__);
#endif
				mark_dummy_rel(joinrel);
				break;
			}
#ifdef __DEBUG__
			elog(LOG, "TEST %s   JOIN_FULL", __func__);
#endif
		...
		case JOIN_SEMI:
				...
				bms_is_subset(sjinfo->min_righthand, rel2->relids))
			{
				...
				{
#ifdef __DEBUG__
					elog(LOG, "TEST %s   JOIN_SEMI(1)  => mark_dummy_rel(joinrel)", __func__);
#endif
					mark_dummy_rel(joinrel);
					break;
				}
#ifdef __DEBUG__
				elog(LOG, "TEST %s   JOIN_SEMI(1)", __func__);
#endif
#ifdef __PG_QUERY_PLAN__
				if (pgqp_adjust_rows
					&& pgqp_enable_adjust_joinrel_rows
								   ...
								   sjinfo) != NULL)
			{
				...
				{
#ifdef __DEBUG__
					elog(LOG, "TEST %s   JOIN_SEMI(2)  => mark_dummy_rel(joinrel)", __func__);
#endif
					mark_dummy_rel(joinrel);
					break;
				}
#ifdef __DEBUG__
				elog(LOG, "TEST %s   JOIN_SEMI(2)", __func__);
#endif
		...
		case JOIN_ANTI:
			...
			{
#ifdef __DEBUG__
				elog(LOG, "TEST %s   JOIN_ANTI  => mark_dummy_rel(joinrel)", __func__);
#endif
				mark_dummy_rel(joinrel);
				break;
			}
			if (restriction_is_constant_false(restrictlist, joinrel, false) &&
				...
				mark_dummy_rel(rel2);
#ifdef __DEBUG__
			elog(LOG, "TEST %s   JOIN_ANTI", __func__);
#endif
#ifdef __PG_QUERY_PLAN__
			if (pgqp_adjust_rows
				&& pgqp_enable_adjust_joinrel_rows
...
#ifndef  __PG_QUERY_PLAN__
/* Ignored this function even if create_append_path is used, because of dummy */
/*
