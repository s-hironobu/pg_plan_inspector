...
#include "pgqp_planagg.h"
#include "pgqp_preunion.h"
#include "pgqp_setrefs.h"
#include "pgqp_subselect.h"
...
#ifdef __PG_QUERY_PLAN__
PlannedStmt *
pgqp_standard_planner(Query *parse, const char *query_string, int cursorOptions,
					  ParamListInfo boundParams)
#else
PlannedStmt *
...
#ifdef __PG_QUERY_PLAN__
	best_path = pgqp_get_cheapest_fractional_path(final_rel, tuple_fraction);
#else
...
#ifdef __PG_QUERY_PLAN__
	top_plan = pgqp_set_plan_references(root, top_plan);
#else
...
#ifdef __PG_QUERY_PLAN__
		lfirst(lp) = pgqp_set_plan_references(subroot, subplan);
#else
...
#ifdef __PG_QUERY_PLAN__
		expr = pgqp_SS_process_sublinks(root, expr, (kind == EXPRKIND_QUAL));
#else
...
#endif
{
	...
	if (parse->setOperations)
	{
		 ...
		 */
#ifdef __PG_QUERY_PLAN__
		current_rel = pgqp_plan_set_operations(root);
#else
	...
	else
	{
		...
		if (parse->hasAggs)
#ifdef __PG_QUERY_PLAN__
			pgqp_preprocess_minmax_aggregates(root);
#else
		 ...
		 */
#ifdef __PG_QUERY_PLAN__
		current_rel = pgqp_query_planner(root, standard_qp_callback, &qp_extra);
#else
...
preprocess_rowmarks(PlannerInfo *root)
{
...
#ifdef __PG_QUERY_PLAN__
		newrc->markType = pgqp_select_rowmark_type(rte, rc->strength);
#else
	...
	foreach(l, parse->rtable)
	{
		...
		newrc->rowmarkId = ++(root->glob->lastRowMarkId);
#ifdef __PG_QUERY_PLAN__
		newrc->markType = pgqp_select_rowmark_type(rte, LCS_NONE);
#else
...
#ifdef __PG_QUERY_PLAN__
RowMarkType
pgqp_select_rowmark_type(RangeTblEntry *rte, LockClauseStrength strength)
#else
RowMarkType
...
#ifdef __PG_QUERY_PLAN__
			pgqp_mark_partial_aggref(newaggref, AGGSPLIT_INITIAL_SERIAL);
#else
...
#ifdef __PG_QUERY_PLAN__
void
pgqp_mark_partial_aggref(Aggref *agg, AggSplit aggsplit)
#else
void
...
#ifdef __PG_QUERY_PLAN__
Path *
pgqp_get_cheapest_fractional_path(RelOptInfo *rel, double tuple_fraction)
#else
Path *
...
#ifndef __PG_QUERY_PLAN__
/* Ignored this because this is only used CLUSTER command. */
/*
...
#ifdef __PG_QUERY_PLAN__
	seqScanPath = pgqp_create_seqscan_path(root, rel, NULL, 0);
#else
...
#ifdef __PG_QUERY_PLAN__
	indexScanPath = pgqp_create_index_path(root, indexInfo,
										   NIL, NIL, NIL, NIL,
...
#ifdef __PG_QUERY_PLAN__
	pgqp_generate_useful_gather_paths(root, rel, true);
#else
...
#ifdef __PG_QUERY_PLAN__
		pgqp_add_paths_to_append_rel(root, partially_grouped_rel,
									 partially_grouped_live_children);
#else
...
#ifdef __PG_QUERY_PLAN__
		pgqp_add_paths_to_append_rel(root, grouped_rel, grouped_live_children);
#else
